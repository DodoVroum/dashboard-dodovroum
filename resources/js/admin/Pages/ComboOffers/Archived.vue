  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <a href="/admin/combo-offers" class="text-sm text-slate-500 hover:text-slate-700 flex items-center gap-1 transition-colors">
            <ChevronLeft class="w-4 h-4" />
            Offres combinées
          </a>
        </div>
        <h1 class="text-xl sm:text-2xl font-bold text-slate-900">Offres combinées archivées</h1>
        <p class="text-sm text-slate-500 mt-0.5">Toutes les offres désactivées sur la plateforme</p>
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div v-if="comboOffers.length === 0 && !error" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-4">Les offres désactivées apparaîtront ici.</p>
        <a href="/admin/combo-offers" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition inline-block text-sm font-medium">
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{{ offer.ownerName || '—' }}</td>
      ...
      <Pagination
        ...
        route-name="admin.combo-offers.archives"
        :filters="{}"
      />
    </div>
  </div>
