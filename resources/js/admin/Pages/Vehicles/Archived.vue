  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <a href="/admin/vehicles" class="text-sm text-slate-500 hover:text-slate-700 flex items-center gap-1 transition-colors">
            <ChevronLeft class="w-4 h-4" />
            Véhicules
          </a>
        </div>
        <h1 class="text-xl sm:text-2xl font-bold text-slate-900">Véhicules archivés</h1>
        <p class="text-sm text-slate-500 mt-0.5">Tous les véhicules désactivés sur la plateforme</p>
      </div>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div v-if="vehicles.length === 0 && !error" class="p-12 text-center">
        ...
        <p class="text-slate-500 mb-4">Les véhicules désactivés apparaîtront ici.</p>
        <a
          href="/admin/vehicles"
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            >
              ...
              <!-- Propriétaire -->
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm text-slate-600">{{ vehicle.ownerName || '—' }}</span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right" @click.stop>
                <div class="flex items-center justify-end gap-2">
                  ...
                  <form :action="`/admin/vehicles/${vehicle.id}/reactivate`" method="POST">
      ...
      <Pagination
        ...
        route-name="admin.vehicles.archives"
        :filters="{}"
      />
    </div>
  </div>
